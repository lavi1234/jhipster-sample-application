...
export class RatingService {
  ...
  protected convertDateFromClient(rating: IRating): IRating {
    const copy: IRating = Object.assign({}, rating, {
      createdAt: rating.createdAt && rating.createdAt.isValid() ? rating.createdAt.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((rating: IRating) => {
        rating.createdAt = rating.createdAt ? moment(rating.createdAt) : undefined;
      });
    }
    return res;
  }
